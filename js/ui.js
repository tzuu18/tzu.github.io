...
const UI = {
        ...
        return `
            <div class="text-center animate-fade-in py-6 md:py-10">
                ...
                <div class="flex flex-col md:flex-row justify-center gap-4 md:gap-6 mt-4 md:mt-8 mb-12 md:mb-16">
                    ...
                    <div onclick="if(window.Router) window.Router.push('folder', 'SUP')" class="bg-white p-6 md:p-8 rounded-3xl shadow-md border border-stone-100 cursor-pointer transform md:hover:-translate-y-2 transition-all duration-300 hover:shadow-xl group w-full md:w-1/3">
                        <div class="bg-amber-50 w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-amber-500 transition-colors">
                            <span class="text-2xl md:text-3xl font-black text-amber-500 group-hover:text-white transition-colors">MT</span>
                        </div>
                        <h2 class="text-xl md:text-2xl font-black text-stone-800 mb-1 md:mb-2">Materi Tambahan</h2>
                        <p class="text-sm md:text-base text-stone-500">Konjugasi, Angka & Waktu</p>
                    </div>
                    ...
                    <div onclick="if(window.Router) window.Router.push('folder', 'N4')" class="bg-white p-6 md:p-8 rounded-3xl shadow-md border border-stone-100 cursor-pointer transform md:hover:-translate-y-2 transition-all duration-300 hover:shadow-xl group w-full md:w-1/3">
                        ...
                        <p class="text-sm md:text-base text-stone-500">Materi Menengah Bawah</p>
                    </div>
                </div>
    ...
    // 2. TAMPILAN FOLDER
    renderFolder: (level) => {
        const isSup = level === 'SUP';
        const start = isSup ? 101 : (level === 'N5' ? 1 : 26);
        const daysLength = isSup ? 7 : 25; 
        const days = Array.from({length: daysLength}, (_, i) => i + start);
        
        const accentText = isSup ? 'text-amber-500' : (level === 'N5' ? 'text-[#E60012]' : 'text-blue-600');
        const badgeBg = isSup ? 'bg-amber-500' : (level === 'N5' ? 'bg-[#E60012]' : 'bg-blue-600');
        const folderName = isSup ? 'Materi Tambahan N5' : `Level ${level}`;
        const labelItem = isSup ? 'Bab' : 'Hari';
        ...
        return `
            <div class="animate-fade-in space-y-6 md:space-y-8 text-left">
                <div class="flex flex-col md:flex-row md:items-center justify-between border-b border-stone-200 pb-4 gap-4">
                    <div>
                        <h2 class="text-2xl md:text-3xl font-black text-stone-900 tracking-tight">Folder <span class="${accentText}">${folderName}</span></h2>
                        <p class="text-sm md:text-base text-stone-500 font-medium mt-1">Pilih materi untuk mulai belajar.</p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                    ${days.map(d => {
                        ...
                        const borderHover = isSup ? 'hover:border-amber-500' : (level === 'N5' ? 'hover:border-[#E60012]' : 'hover:border-blue-600');
                        const displayNum = isSup ? (d - 100) : d;
                        ...
                        if (hasData) {
                            ...
                            <div class="bg-stone-50 border-2 border-dashed border-stone-200 p-5 md:p-6 rounded-3xl flex flex-col h-full cursor-not-allowed opacity-60">
                                ...
                                <p class="text-stone-400 text-xs md:text-sm leading-relaxed mt-auto">
                                    Materi ini masih dalam tahap penyusunan.
                                </p>
                            </div>`;
                        }
                    }).join('')}
    ...
    // 3. TAMPILAN MATERI (DENGAN LOGIKA FOLDER SUPLEMEN BAB 102)
    renderLesson: (level, day) => {
        ...
        const isSup = level === 'SUP';
        const accentColor = isSup ? 'text-amber-500' : (level === 'N5' ? 'text-[#E60012]' : 'text-blue-600');
        const borderAccent = isSup ? 'border-amber-500' : (level === 'N5' ? 'border-[#E60012]' : 'border-blue-600');

        // --- HTML UNTUK KOTOBA ---
        let kotobaContent = '';
        if (level === 'N5' && day === 1) {
            // N5 Hari 1 Accordion Hiragana/Katakana
            kotobaContent = `
            ...
            `;
        } else if (level === 'SUP' && day === 104) {
            // Khusus Suplemen Bab 102 (Kata Bantu Bilangan dengan 16 Folder Accordion)
            kotobaContent = `
                <div class="space-y-3 md:space-y-4">
                    <p class="text-[10px] md:text-xs font-black text-amber-600 uppercase tracking-widest mb-2 md:mb-4">Klik Folder Kategori untuk Membuka:</p>
                    ${lesson.kotoba.map((group, idx) => `
                        <div class="bg-white rounded-2xl md:rounded-3xl border border-amber-100 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
                            <button onclick="UI.toggleDay1Accordion('sup-102-${idx}')" class="w-full flex items-center justify-between p-4 md:p-6 hover:bg-amber-50/50 transition-all text-left">
                                <div class="flex items-center gap-3 md:gap-4">
                                    <div class="w-10 h-10 md:w-12 md:h-12 bg-amber-100 text-amber-600 rounded-xl md:rounded-2xl flex items-center justify-center text-xl md:text-2xl shadow-inner">
                                        ${group.icon}
                                    </div>
                                    <div>
                                        <h4 class="font-black text-stone-800 tracking-tight text-sm md:text-base">${group.kategori}</h4>
                                        <p class="text-[10px] text-amber-500 font-bold uppercase tracking-widest mt-0.5">Buka Folder</p>
                                    </div>
                                </div>
                                <span id="icon-sup-102-${idx}" class="text-amber-400 transform transition-transform duration-300 text-xl">▼</span>
                            </button>
                            
                            <div id="sup-102-${idx}" class="hidden p-4 md:p-6 bg-amber-50/30 border-t border-amber-50 animate-fade-in">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                                    ${group.list.map(k => `
                                        <div class="flex flex-col border border-amber-100 p-3 md:p-4 rounded-xl bg-white shadow-sm hover:border-amber-300 transition-colors">
                                            <div class="flex justify-between items-center mb-1 gap-2">
                                                <span class="text-xl md:text-2xl font-black font-jp text-amber-600">${k.kanji}</span>
                                                <span class="font-bold text-stone-700 text-right text-xs md:text-sm leading-tight">${k.arti}</span>
                                            </div>
                                            <p class="text-[10px] md:text-xs text-amber-500 font-bold uppercase tracking-widest">${k.kana}</p>
                                        </div>
                                    `).join('')}
                                </div>
                            </div>
            ...
            // List Kotoba biasa untuk materi lain
            const colorClass = isSup ? 'amber' : 'emerald';
            kotobaContent = `
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                    ${lesson.kotoba.map(k => `
                        <div class="flex flex-col border border-${colorClass}-50 p-4 md:p-5 rounded-2xl bg-${colorClass}-50/30 hover:bg-${colorClass}-50 transition-all group">
                            <div class="flex justify-between items-center mb-2 gap-2">
                                <span class="text-2xl md:text-3xl font-black font-jp text-${colorClass}-600 group-hover:text-${colorClass}-700">${k.kanji}</span>
                                <span class="font-bold text-stone-700 text-right text-sm md:text-base leading-tight">${k.arti}</span>
                            </div>
                            <p class="text-[10px] md:text-xs text-${colorClass}-500 font-bold uppercase tracking-widest">${k.kana}</p>
                        </div>
                    `).join('')}
                </div>
            `;
        ...
        // --- HTML UNTUK DOKKAI, CHOUKAI, KANJI (Disembunyikan jika Suplemen) ---
        let dokkaiHtml = '', choukaiHtml = '', kanjiHtml = '';
        if (!isSup) {
            // (Logika Dokkai, Choukai, Kanji tetap sama seperti sebelumnya, tapi kita lewatkan jika isSup = true)
            // Dokkai
            if (lesson.dokkai) {
                dokkaiHtml = `
                    <div class="bg-white p-5 md:p-8 rounded-3xl border border-blue-100 shadow-sm">
                        <div class="text-center mb-6"><span class="text-4xl block mb-2 opacity-80">📖</span><h3 class="text-xl md:text-2xl font-black font-jp text-blue-900">${lesson.dokkai.title}</h3></div>
                        <div class="bg-blue-50/50 p-5 md:p-8 rounded-2xl border border-blue-200 mb-6 overflow-x-auto"><p class="text-lg md:text-xl leading-loose md:leading-[2.5] font-bold font-jp text-blue-900 whitespace-pre-line min-w-[250px]">${lesson.dokkai.text}</p></div>
                        <div class="bg-blue-50/30 p-5 md:p-6 rounded-2xl border border-blue-100"><span class="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-100 px-2 py-1 rounded-md mb-3 inline-block">Terjemahan</span><p class="text-sm md:text-base text-stone-600 leading-relaxed whitespace-pre-line">${lesson.dokkai.translation}</p></div>
                    </div>`;
            } else { dokkaiHtml = `<div class="bg-blue-50/50 border-2 border-dashed border-blue-200 p-8 md:p-12 rounded-3xl text-center"><span class="text-5xl md:text-6xl mb-4 block opacity-30 grayscale">📚</span><h4 class="text-xl md:text-2xl font-black text-blue-400 mb-2">Cerita Belum Tersedia</h4></div>`; }
            
            // Choukai
            if (lesson.choukai) {
                let mediaHtml = lesson.choukai.youtubeId ? `<div class="w-full max-w-2xl aspect-video rounded-2xl overflow-hidden shadow-md border-2 border-amber-200 bg-black mx-auto"><iframe width="100%" height="100%" src="https://www.youtube.com/embed/${lesson.choukai.youtubeId}?rel=0" frameborder="0" allowfullscreen></iframe></div>` : `<div class="w-full max-w-md bg-white p-4 rounded-xl border border-amber-200 shadow-sm flex items-center gap-4"><button class="w-10 h-10 bg-amber-500 text-white rounded-full flex items-center justify-center shadow-md">▶</button><div class="flex-1 h-2 bg-amber-100 rounded-full"></div><span class="text-xs font-bold text-amber-600">0:00</span></div>`;
                choukaiHtml = `
                    <div class="bg-white p-5 md:p-8 rounded-3xl border border-amber-100 shadow-sm">
                        <div class="text-center mb-6"><span class="text-4xl block mb-2 opacity-80">🎧</span><h3 class="text-xl md:text-2xl font-black font-jp text-amber-900">${lesson.choukai.title}</h3></div>
                        <div class="bg-amber-50/50 p-5 md:p-8 rounded-2xl border border-amber-200 mb-6 flex flex-col items-center gap-4 text-center">${mediaHtml}${lesson.choukai.text ? `<p class="text-lg md:text-xl font-bold font-jp text-amber-900 whitespace-pre-line mt-4">${lesson.choukai.text}</p>` : ''}</div>
                        <div class="bg-amber-50/30 p-5 md:p-6 rounded-2xl border border-amber-100"><span class="text-[10px] font-black text-amber-600 uppercase tracking-widest bg-amber-100 px-2 py-1 rounded-md mb-3 inline-block">Transkrip / Terjemahan</span><p class="text-sm md:text-base text-stone-600 leading-relaxed whitespace-pre-line">${lesson.choukai.translation}</p></div>
                    </div>`;
            } else { choukaiHtml = `<div class="bg-amber-50/50 border-2 border-dashed border-amber-200 p-8 md:p-12 rounded-3xl text-center"><span class="text-5xl md:text-6xl mb-4 block opacity-30 grayscale">🎧</span><h4 class="text-xl md:text-2xl font-black text-amber-400 mb-2">Audio Belum Tersedia</h4></div>`; }

            // Kanji
            if (lesson.kanji) {
                kanjiHtml = `
                    <div class="bg-white p-5 md:p-8 rounded-3xl border border-teal-100 shadow-sm">
                        <div class="text-center mb-6"><span class="text-4xl block mb-2 opacity-80">✍️</span><h3 class="text-xl md:text-2xl font-black font-jp text-teal-900">${lesson.kanji.title || 'Belajar Kanji'}</h3></div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            ${lesson.kanji.list.map(k => `<div class="bg-teal-50/50 p-5 rounded-2xl border border-teal-100 flex flex-col items-center justify-center group"><span class="text-5xl font-black font-jp text-teal-600 mb-3">${k.karakter}</span><div class="text-center space-y-1"><p class="text-xs font-bold text-teal-500 uppercase tracking-widest">Kunyomi</p><p class="text-sm text-stone-700">${k.kunyomi || '-'}</p><p class="text-xs font-bold text-teal-500 uppercase tracking-widest mt-2">Onyomi</p><p class="text-sm text-stone-700">${k.onyomi || '-'}</p><p class="text-sm font-bold text-stone-900 mt-2 bg-white px-3 py-1 rounded-lg border border-teal-100">${k.arti}</p></div></div>`).join('')}
                        </div>
                    </div>`;
            } else { kanjiHtml = `<div class="bg-teal-50/50 border-2 border-dashed border-teal-200 p-8 md:p-12 rounded-3xl text-center"><span class="text-5xl md:text-6xl mb-4 block opacity-30 grayscale">✍️</span><h4 class="text-xl md:text-2xl font-black text-teal-400 mb-2">Kanji Belum Tersedia</h4></div>`; }
        }
        ...
        return `
            <div class="fade-in space-y-6 md:space-y-8 max-w-4xl mx-auto text-left pb-10">
                ...
                <div class="bg-white p-5 md:p-6 rounded-[2rem] border border-stone-100 shadow-sm">
                    <button onclick="Router.push('folder', '${level}')" class="text-stone-400 hover:text-stone-900 font-bold mb-3 flex items-center gap-2 text-xs md:text-sm uppercase tracking-widest">
                        ← Kembali
                    </button>
                ...
                <div id="content-kotoba" class="block fade-in space-y-6">
                    <div class="bg-white p-5 md:p-8 rounded-3xl border border-stone-100 shadow-sm">
                        <h3 class="text-xl md:text-2xl font-black text-stone-800 mb-4 md:mb-6 flex items-center">
                            ${isSup 
                                ? `<span class="bg-amber-100 text-amber-600 px-3 py-1 rounded-lg mr-3 text-xs uppercase tracking-tighter">Materi</span> Referensi Tambahan` 
                                : `<span class="bg-emerald-100 text-emerald-600 px-3 py-1 rounded-lg mr-3 text-xs uppercase tracking-tighter">Bagian 1</span> Kosakata Baru (Kotoba)`
                            }
                        </h3>
                        ${kotobaContent}
                    </div>
                    ...
                    <div class="text-center pt-4 md:pt-6 border-t-2 border-dashed border-stone-200">
                        <button onclick="if(window.Quiz) Quiz.start('${level}', ${day}, 'kotoba')" class="w-full md:w-auto bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all text-base md:text-lg">
                            📝 Uji Kosakata
                        </button>
                    </div>
                    `}
                ...
                <div id="content-bunpou" class="hidden fade-in space-y-6">
                    <h3 class="text-xl md:text-2xl font-black text-stone-800 mb-4 md:mb-6 flex items-center">
                        <span class="bg-purple-100 text-purple-600 px-3 py-1 rounded-lg mr-3 text-xs uppercase tracking-tighter">Bagian 2</span> Tata Bahasa (Bunpou)
                    </h3>
                    ${lesson.bunpou.map(b => `
                    ...
                    `).join('')}
                    <div class="text-center pt-4 md:pt-6 border-t-2 border-dashed border-stone-200"><button onclick="if(window.Quiz) Quiz.start('${level}', ${day}, 'bunpou')" class="w-full md:w-auto bg-purple-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-purple-200 hover:bg-purple-700 transition-all text-base md:text-lg">📝 Uji Tata Bahasa</button></div>
                </div>

                <div id="content-dokkai" class="hidden fade-in space-y-6">
                    <h3 class="text-xl md:text-2xl font-black text-stone-800 mb-4 md:mb-6 flex items-center"><span class="bg-blue-100 text-blue-600 px-3 py-1 rounded-lg mr-3 text-xs uppercase tracking-tighter">Bagian 3</span> Membaca Cerita (Dokkai)</h3>
                    ${dokkaiHtml}
                    <div class="text-center pt-4 border-t-2 border-dashed border-stone-200"><button onclick="if(window.Quiz) Quiz.start('${level}', ${day}, 'dokkai')" class="w-full md:w-auto bg-blue-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all text-base md:text-lg">📝 Uji Pemahaman Cerita</button></div>
                </div>

                <div id="content-choukai" class="hidden fade-in space-y-6">
                    <h3 class="text-xl md:text-2xl font-black text-stone-800 mb-4 md:mb-6 flex items-center"><span class="bg-amber-100 text-amber-600 px-3 py-1 rounded-lg mr-3 text-xs uppercase tracking-tighter">Bagian 4</span> Mendengar (Choukai)</h3>
                    ${choukaiHtml}
                    <div class="text-center pt-4 border-t-2 border-dashed border-stone-200"><button onclick="if(window.Quiz) Quiz.start('${level}', ${day}, 'choukai')" class="w-full md:w-auto bg-amber-500 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-amber-200 hover:bg-amber-600 transition-all text-base md:text-lg">📝 Uji Pendengaran</button></div>
                </div>

                <div id="content-kanji" class="hidden fade-in space-y-6">
                    <h3 class="text-xl md:text-2xl font-black text-stone-800 mb-4 md:mb-6 flex items-center"><span class="bg-teal-100 text-teal-600 px-3 py-1 rounded-lg mr-3 text-xs uppercase tracking-tighter">Bagian 5</span> Menulis Kanji (Kanji)</h3>
                    ${kanjiHtml}
                    <div class="text-center pt-4 border-t-2 border-dashed border-stone-200"><button onclick="if(window.Quiz) Quiz.start('${level}', ${day}, 'kanji')" class="w-full md:w-auto bg-teal-500 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-teal-200 hover:bg-teal-600 transition-all text-base md:text-lg">📝 Uji Kanji</button></div>
                </div>
                `}
            </div>`;
    },
    ...
    // 7. LOGIKA KHUSUS ACCORDION (HARI 1 N5 & BAB 102 SUPLEMEN)
